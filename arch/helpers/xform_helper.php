...
if (!function_exists('image_slideshow')){
    function image_slideshow($images) {
        ...
        <div class="image-show">
            <a href="<?php echo site_url($module.'/show_image/' . $id) ?>" class="msgbox">
                <?php for ($i = 0; $i < 5; $i++): ?>
                    <?php $image = @$images[$i] ?>
                    <?php if(!empty($image['uri']) || $i != 0): ?>
                    <i class="image-container data-image" style="background-image: url(<?php echo data_url(@$image['uri']) ?>);"></i>
                    <?php else: ?>
                    <i class="image-container data-image" style="background-image: url(<?php echo theme_url('images/logodephub.png') ?>);"></i>
                    <?php endif ?>
                <?php endfor ?>
            </a>
        </div>
