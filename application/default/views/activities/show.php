...
<form enctype="multipart/form-data" action="<?php echo current_url() ?>" method="post" class="ajaxform">
    <fieldset>
        ...
        <div>
            <label class="mandatory"><?php echo l('Activities Category') ?></label>
            <?php echo form_dropdown('category_id', $activities_options) ?>
        </div>
    </fieldset>
    <fieldset>
    ...
    <fieldset>
        ...
        <div>
            <label class="mandatory"><?php echo l('Title (Chinese)') ?></label>
            <input type="text" value="<?php echo set_value('title_cn') ?>" name="title_cn" placeholder="Title (Chinese)" />
        </div>
        <label class="mandatory"><?php echo l('Description (Chinese)') ?></label>
        <div>
            <textarea class="ckeditor" name="description_cn" placeholder="Description (Chinese)" /><?php echo set_value('description_cn') ?></textarea>
        </div>
    </fieldset>
    <fieldset>
           ...
           <div>
                <?php foreach ($images as $key => $img) :?>
                    <div class="thumbs thumbnail">
                        <img src="<?php echo base_url('timthumb.php?src=')?><?php echo base_url('data/area/image/original') . '/' . $img['file_name'] ?>&w=100&h=100" alt=""/>
